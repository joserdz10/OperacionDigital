...
export async function runDiscovery(territoryCode, identityCode, period = '6h') {
    const brain = await one('SELECT id,name,timezone FROM territory_brains WHERE code=$1', [territoryCode]);
    const identity = await one('SELECT id,name FROM identities WHERE code=$1', [identityCode]);
    if (!brain || !identity)
        throw new Error('Active State Brain or identity not found.');
    const topics = await query('SELECT code,name,priority,semantic_queries FROM topics WHERE territory_brain_id=$1 AND monitor_enabled=true ORDER BY priority DESC LIMIT 40', [brain.id]);
    const profiles = await query('SELECT code,name,profile_type,importance,aliases FROM profiles WHERE territory_brain_id=$1 AND monitor_enabled=true ORDER BY importance DESC LIMIT 50', [brain.id]);
    const sources = await query('SELECT name,domain,source_type,reliability_score,metadata FROM sources WHERE territory_brain_id=$1 AND monitor_enabled=true ORDER BY reliability_score DESC LIMIT 40', [brain.id]);
    ...
    for (const s of stories) {
      ...
      RETURNING id,story_number,title`, [
        ...
        for (const t of (s.topics || [])) {
            const topic = await one('SELECT id FROM topics WHERE territory_brain_id=$1 AND (code=$2 OR lower(name)=lower($3)) LIMIT 1', [brain.id, t.code || '', t.name || '']);
            if (topic)
                await query('INSERT INTO story_topics(story_id,topic_id,relevance_score) VALUES($1,$2,$3) ON CONFLICT DO NOTHING', [story.id, topic.id, clamp(t.score)]);
        }
        for (const p of (s.profiles || [])) {
            const profile = await one('SELECT id FROM profiles WHERE territory_brain_id=$1 AND (code=$2 OR lower(name)=lower($3)) LIMIT 1', [brain.id, p.code || '', p.name || '']);
            if (profile)
                await query('INSERT INTO story_profiles(story_id,profile_id,relation,relevance_score) VALUES($1,$2,$3,$4) ON CONFLICT DO NOTHING', [story.id, profile.id, p.relation || 'related', clamp(p.score)]);
        }

...
export async function generateBundle(story, kind = 'todo') {
    const details = await storyDetail(story.id);
    const mediaDna = await one('SELECT * FROM media_dna WHERE identity_id=$1 AND is_current=true ORDER BY version DESC LIMIT 1', [story.identity_id]);
    const visualDna = await one('SELECT * FROM visual_dna WHERE identity_id=$1 AND is_current=true ORDER BY version DESC LIMIT 1', [story.identity_id]);
    const safeClaims = details.claims.filter((c) => ['verified', 'supported'].includes(c.verification_status));
    const riskyClaims = details.claims.filter((c) => !['verified', 'supported'].includes(c.verification_status));

...
export async function inbox(identityCode, limit = 12) {
    return query(`
...
export async function findStory(identityCode, ref, lastStoryId) {
    if (!ref && lastStoryId)
        ref = lastStoryId;
    if (!ref)
        return null;
...
export async function storyDetail(storyId) {
    const [topics, profiles, sources, claims] = await Promise.all([
        query('SELECT t.code,t.name,st.relevance_score FROM story_topics st JOIN topics t ON t.id=st.topic_id WHERE st.story_id=$1 ORDER BY st.relevance_score DESC', [storyId]),
        query('SELECT p.code,p.name,sp.relation,sp.relevance_score FROM story_profiles sp JOIN profiles p ON p.id=sp.profile_id WHERE sp.story_id=$1 ORDER BY sp.relevance_score DESC', [storyId]),
        query('SELECT source_name,source_url,source_tier,is_primary FROM story_sources WHERE story_id=$1 ORDER BY is_primary DESC,source_name', [storyId]),
        query('SELECT claim_text,verification_status,confidence_score,attribution FROM story_claims WHERE story_id=$1 ORDER BY confidence_score DESC', [storyId]),
    ]);

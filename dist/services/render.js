...
function inferSection(text) {
    ...
    if (/metro|movilidad|transporte|viaducto|línea|linea|gonzalitos|tr[aá]fico|carretera/.test(t))
        return 'MOVILIDAD';
    if (/lluvia|clima|tormenta|inund|protecci[oó]n civil/.test(t))
        return 'PROTECCIÓN CIVIL';
    if (/cateo|fiscal[ií]a|seguridad|fuerza civil|delito|homicidio|robo|amenaza|detenid/.test(t))
        return 'SEGURIDAD';
    if (/sheinbaum|gobierno|samuel|congreso|federal|alcald|diputad|elecci[oó]n/.test(t))
        return 'POLÍTICA';
    if (/inversi[oó]n|industria|empresa|empleo|nearshoring|econom[ií]a/.test(t))
        return 'ECONOMÍA';
    if (/tigres|rayados|f[uú]tbol|deporte/.test(t))
        return 'DEPORTES';
    return 'NUEVO LEÓN';
}
function inferLocation(text) {
    ...
    for (const [re, label] of locations)
        if (re.test(t))
            return label;
    return 'NUEVO LEÓN';
}
function kickerFor(story, spec) {
    ...
    if (location === section)
        return location;
    if (location === 'NUEVO LEÓN' && section === 'NUEVO LEÓN')
        return 'NUEVO LEÓN';
...
function safeHttpUrl(value) {
    ...
    try {
        ...
        if (!['http:', 'https:'].includes(u.protocol) || blockedHostname(u.hostname))
            return null;
        return u;
    }
    catch {
...
async function fetchSourceImage(storyId, tmpDir) {
    const sources = await query(`SELECT source_name, source_url, source_tier, is_primary
    ...
    for (const source of sources) {
        ...
        try {
            ...
            if (!imageType.startsWith('image/') || imageType.includes('svg'))
                continue;
            const ab = await imageRes.arrayBuffer();
            if (ab.byteLength < 10_000 || ab.byteLength > 10_000_000)
                continue;
...
async function prepareSourceImage(photoPath, tmpDir) {
    if (!/\.webp$/i.test(photoPath))
        return photoPath;
    if (!hasCommand('dwebp'))
...
async function makePhotoBase(photoPath, outPath, width, height) {
    await runMagick([
        'convert', photoPath,
        '-auto-orient',
...
async function makeScaledAsset(asset, outPath, width, height) {
    const geometry = height ? `${width}x${height}` : `${width}x`;
    await runMagick(['convert', asset, '-resize', geometry, outPath]);
}
async function compositeAsset(base, asset, out, x, y) {
    await runMagick(['convert', base, asset, '-gravity', 'northwest', '-geometry', `+${x}+${y}`, '-compose', 'over', '-composite', out]);
}
async function renderStoryLike(photoPath, outPath, story, spec, pieceType, sourceName, tmpDir) {
    const c = brandColors();
...
async function renderFeed(photoPath, outPath, story, spec, pieceType, sourceName, tmpDir) {
    const c = brandColors();
    ...
    args.push('-fill', c.ink, '-font', 'DejaVu-Serif-Bold', '-pointsize', titlePoint, '-interline-spacing', '-6', '-gravity', 'northwest', '-annotate', `+${margin}+${titleY}`, title, '-fill', '#2D3A3E', '-font', 'DejaVu-Sans', '-pointsize', bodyPoint, '-interline-spacing', '4', '-gravity', 'southwest', '-annotate', `+${margin}+${footerH + 52}`, secondary, '-fill', c.copper, '-draw', `rectangle ${margin},${bodyBottom - 36} ${margin + 90},${bodyBottom - 29}`, '-fill', c.muted, '-font', 'DejaVu-Sans-Bold', '-pointsize', '14', '-gravity', 'southwest', '-annotate', `+${margin}+${footerH + 18}`, source, '-fill', c.green, '-draw', `rectangle 0,${bodyBottom} ${width},${height}`, '-fill', c.ivory, '-font', 'DejaVu-Sans-Bold', '-pointsize', '16', '-gravity', 'south', '-annotate', '+0+42', 'SERIO  •  CONFIABLE  •  EDITORIAL  •  REGIONAL', '-quality', '94', stage2);
...
async function renderQuote(outPath, story, spec, tmpDir) {
    const c = brandColors();
...
export async function renderPiecePng({ story, contentPiece, pieceType }) {
